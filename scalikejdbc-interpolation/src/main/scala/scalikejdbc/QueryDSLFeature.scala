...
trait QueryDSLFeature { self: SQLInterpolationFeature with SQLSyntaxSupportFeature =>
  ...
  case class InsertSQLBuilder(override val sql: SQLSyntax) extends SQLBuilder[UpdateOperation] {
    ...
    def namedValues(columnsAndValues: (SQLSyntax, EssentialParameterBinder)*): InsertSQLBuilder = {
  ...
  case class UpdateSQLBuilder(override val sql: SQLSyntax) extends SQLBuilder[UpdateOperation]
    ...
    def set(tuples: (SQLSyntax, EssentialParameterBinder)*): UpdateSQLBuilder = set(sqls.csv(tuples.map(each => sqls"${each._1} = ${each._2}"): _*))
